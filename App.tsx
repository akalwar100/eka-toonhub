...
import { useLastSynced } from '@/data/liveData';
...
interface RadialOrbitalTimelineProps {
  ...
  onClose: () => void;
  /** Whether region-team data (a real sheet, or the local dev mock backend)
   *  is wired up. Shown as a small badge in the header so it's obvious
   *  whether you're looking at live operational data or hardcoded sample
   *  data. */
  isLive?: boolean;
  /** Text shown inside the live badge when isLive is true — lets the
   *  caller distinguish "Live region data" (real sheet) from
   *  "Live (local dev mock)" without this component needing to know which
   *  backend is in play. */
  liveLabel?: string;
}
...
export default function RadialOrbitalTimeline({
  ...
  const { lastSynced, justUpdated } = useLastSynced();
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-5 sm:px-10 py-5 z-50">
        <div className="flex items-center gap-3">
          ...
          <span
            className={`hidden sm:inline-flex items-center gap-1 text-[9px] font-mono uppercase tracking-widest px-2 py-0.5 rounded-full border transition-all duration-300 ${
              isLive
                ? justUpdated
                  ? 'text-emerald-200 border-emerald-300/60 bg-emerald-400/20 scale-105'
          ...
          >
            ...
            />
            {isLive ? liveLabel : 'Sample data'}
          </span>
          {isLive && <SyncedAgo lastSynced={lastSynced} justUpdated={justUpdated} />}
        </div>
        <button
          onClick={onClose}
...
/** Small "synced Xs ago" readout, ticking every second, flashing the accent
 *  color briefly whenever a fresh sync lands — this is the visible proof
 *  that the auto-update loop is actually running, not just a static badge. */
function SyncedAgo({ lastSynced, justUpdated }: { lastSynced: Date | null; justUpdated: boolean }) {
  ...
  if (!lastSynced) {
    return <span className="hidden md:inline text-[9px] font-mono text-white/30">syncing…</span>;
  }

  const seconds = Math.max(0, Math.floor((Date.now() - lastSynced.getTime()) / 1000));
  const label = seconds < 5 ? 'just now' : seconds < 60 ? `${seconds}s ago` : `${Math.floor(seconds / 60)}m ago`;

  return (
    <span
      className={`hidden md:inline text-[9px] font-mono uppercase tracking-widest transition-colors duration-300 ${
        justUpdated ? 'text-emerald-200' : 'text-white/30'
      }`}
    >
      synced {label}
    </span>
